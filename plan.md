...
2. Install dependencies:
   ...
   ```
3. Configure OpenAI API key (uses low-cost models `text-embedding-3-small` and `gpt-image-embedding-1`):
   ```bash
   cp env.example .env
...
- Text-to-image search (after ingesting images via organize):  
  `python main.py search_image "sunset on the beach" --top_k 5`
- Batch ingest & optional auto-foldering:  
  `python main.py organize_folder ./my_folder --topics "CV"`
- Initialize git repo helper:  
  `python main.py init_git`

Data is stored locally under `data/index` via ChromaDB; PDFs are chunked before embedding. Images are embedded via OpenAI image embedding API.

### Notes
- API key is read from environment (`OPENAI_API_KEY`). Do not commit real keys.
- `.gitignore` excludes `data/`, `.env`, caches, and venv artifacts.
